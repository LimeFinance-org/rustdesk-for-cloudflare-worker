...
export class Hbbr extends DurableObject {
  ...
  handleRequestRelay(req: rendezvous.RequestRelay, socket: WebSocket) {
    // Reject relay attempts unless the client presents the configured licence key.
    const relayKey = (this.env as { RELAY_KEY?: string }).RELAY_KEY
    if (relayKey && req.licenceKey !== relayKey) {
      console.log(`relay auth failed: licence key mismatch`)
...
export class Hbbs extends DurableObject {
  ...
  async fetch(req: Request): Promise<Response> {
    // Remember where clients reached us, so relay URLs can fall back to this
    // origin when the HBBS_RELAY_URL secret is not configured.
  ...
  handlePunchHoleRequest(req: rendezvous.PunchHoleRequest, socket: WebSocket) {
    ...
    // Relay address clients should dial. Prefer the HBBS_RELAY_URL secret;
    // otherwise fall back to the origin this request arrived on.
    const relayUrl = (this.env as { HBBS_RELAY_URL?: string }).HBBS_RELAY_URL || this.origin || 'ws://localhost'
  ...
  handleRegisterPk(req: rendezvous.RegisterPk, socket: WebSocket) {
    ...
    // Reject new registrations when the session table is full, unless it is
    // a re-registration of an already known id. Guards against mass abuse.
    const maxSessions = Number((this.env as { MAX_SESSIONS?: string }).MAX_SESSIONS) || 100
    if (!this.sessions.has(peerId) && this.sessions.size >= maxSessions) {
      console.log(`register rejected: session table full (${maxSessions})`)
      socket.close(4003, 'server full')
      return
    }
    const meta = socket.deserializeAttachment()
